...
def run_problems(problem, rhc_max_attempts, decay_rate, sa_max_attempts, ga_max_attempts, GA_popSize, GA_pop_breed_pec, mimic_max_attempts, mimic_popSize, mimic_keepPct, title):
    # with multiprocessing.Pool() as pool:
    ...
    start_time =time.time()
    _, _, fitness_curve = random_hill_climb(problem, max_attempts=rhc_max_attempts, max_iters=rhc_max_attempts, curve = True)
    rhc_time.append(time.time() - start_time)
    for i in range(len(fitness_curve)):
        rhc_fitness.append(fitness_curve[i][0])
    ...
    start_time =time.time()
    _, _, fitness_curve = simulated_annealing(problem, schedule=exp_decay, max_attempts=sa_max_attempts, max_iters=sa_max_attempts, curve=True)
    sa_time.append(time.time() - start_time)
    for i in range(len(fitness_curve)):
        sa_fitness.append(fitness_curve[i][0])
    ...
    start_time =time.time()
    _, _, fitness_curve = genetic_alg(problem, pop_size=GA_popSize, pop_breed_percent=GA_pop_breed_pec, max_attempts=ga_max_attempts, max_iters=ga_max_attempts, curve=True)
    ga_time.append(time.time() - start_time)
    for i in range(len(fitness_curve)):
        ga_fitness.append(fitness_curve[i][0])
    ...
    start_time =time.time()
    _, _, fitness_curve = mimic(problem, pop_size=mimic_popSize, keep_pct=mimic_keepPct, max_attempts=mimic_max_attempts, max_iters=mimic_max_attempts, curve=True)
    mimic_time.append(time.time() - start_time)
    for i in range(len(fitness_curve)):
        mimic_fitness.append(fitness_curve[i][0])
        ...
        # time_spent, fitness = async_rhc_fitnesses.get()
        # rhc_fitness.append(fitness)
        # rhc_time.append(time_spent)
        # log(f'{counter} / {total} is done')
        # counter += 1

        # rhc_fitness = np.concatenate((rhc_fitness[0]), axis=0)
        ...
        # sa_fitness = np.concatenate((sa_fitness[0]), axis=0)
        ...
        # ga_fitness = np.concatenate((ga_fitness[0]), axis=0)
        ...
        # mimic_fitness = np.concatenate((mimic_fitness[0]), axis=0)
        # mimic_fitness = mimic_fitness[:,0]
        
    rhc_iterations = np.arange(1, rhc_max_attempts + 1)
    sa_iterations = np.arange(1, sa_max_attempts + 1)
    ga_iterations = np.arange(1, ga_max_attempts+1)
    mimic_iterations = np.arange(1, mimic_max_attempts + 1)
    ...
    plt.ylabel("fitness")
    plt.grid()
    plt.plot(rhc_iterations, np.array(rhc_fitness), label = 'RHC')
    plt.plot(sa_iterations, np.array(sa_fitness), label = 'SA')
    plt.plot(ga_iterations, np.array(ga_fitness), label = 'GA')
    plt.plot(mimic_iterations, np.array(mimic_fitness), label = 'MIMIC')
    plt.legend(loc="best")
    plt.savefig('images/'+title+" fitness curve")

    sys.stdout = open('images/'+title +" time.txt", "w")
